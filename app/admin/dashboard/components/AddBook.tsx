...
export default function AddBook({ onSuccess }: { onSuccess: () => void }) {
    ...
    const handleUpload = async () => {
        ...
        if (isFree === 'false' && !price) {
            alert('Vui lòng nhập giá tiền cho sách trả phí!');
            return;
        }
            ...
            <div className="space-y-8">
                ...
                {isFree === 'false' && (
                    <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                        <label className="text-[13px] font-bold text-slate-700 uppercase tracking-wider ml-1">Giá bán (VNĐ)</label>
                        <div className="relative">
                            <input
                                type="text"
                                className="w-full px-5 py-4 pl-12 rounded-xl border border-amber-200 bg-amber-50/30 focus:bg-white focus:ring-4 focus:ring-amber-50 focus:border-amber-500 outline-none transition-all placeholder:text-slate-400 font-bold text-amber-900"
                                placeholder="Nhập số tiền..."
                                value={price ? new Intl.NumberFormat('vi-VN').format(Number(price)) : ''}
